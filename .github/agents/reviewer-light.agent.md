...
- Logic change detected in a `format:` commit
- Production code changed in a `test:` commit
- Non-doc files changed in a `docs:` commit

**Rules:**
- Always end with exactly `[PASS]` or `[FAIL]` on its own line
- Keep the review short — this is a sanity check, not a deep review
